...
def get_line_node_at_end(s, x0, y0, hdg, length):
    # x = x0 + length * np.cos(hdg)
    # y = y0 + length * np.sin(hdg)
    node = get_geometry_line_node(s, x0, y0, hdg, np.abs(length))
    return node
...
if __name__=="__main__":
    ...
    # s_list = np.cumsum(length_list) + line_road_length
    ...
    # Add first line road
    # node = get_line_node_at_end(0, xs[0], ys[0], hdg_list[0], -1*line_road_length)
    # planView.append(node)

    # Set road length
    road.attrib["length"] = str(s_list[-1] + line_road_length) # 头的那个已经算在length_list里面加过了，所以只用加一个就行
    ...
    # Add last line road
    node = get_line_node_at_end(s_list[-1], xs[-1], ys[-1], hdg_list[-1] + np.pi/2, line_road_length)
    planView.append(node)
