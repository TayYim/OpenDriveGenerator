...
def interpolate(x, y):
    ...
    der_1_vals, _ = splev(unew, pos_tck, der=1)
    
    
    return new_x_vals, new_y_vals, der_1_vals
...
if __name__=="__main__":
    ...
    # Input x/y points
    old_xs= [0, 30, 40, 50, 150, 155, 185]
    old_ys= [0, 20, 30, 40, 100, 180, 210 ]
    # old_xs= [40, 50, 150, 155 ]
    # old_ys= [30, 40, 100, 180 ]

    xs, ys, der_1_vals = interpolate(old_xs, old_ys)
    xs = np.array(xs)
    ys = np.array(ys)
    der_1_vals = np.array(der_1_vals)
    ...
    # Get break points
    break_indexs = [0]
    positive_points = np.where(np.diff(ys, n=2)>0)[0]
    for i in range(len(positive_points)-1):
        if positive_points[i] + 1 != positive_points[i+1]:
            break_indexs.append(positive_points[i])
            break_indexs.append(positive_points[i+1])
    break_indexs.append(len(xs)) # 包前不包后，这里不-1
    n_segments = len(break_indexs) - 1
    ...
    # A random choice
    index_point_1 = 2
    index_point_2 = 100 # TODO: dynamic


    for segment_id in range(n_segments):
        index_start = break_indexs[segment_id]
        index_end = break_indexs[segment_id+1] - 1
